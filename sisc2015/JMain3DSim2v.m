...
ylength=mdim;%radius of the tube
...
co = min(4, abs(0.09/(0.0120*log(w)+0.0465)));%correction to keep strain amp const over various frequencies
...
k=sqrt(w*rho0/(2*visc0))*charLength;
i=sqrt(-1);
ua=sqrt(real(sinh(k*y*(1+i))/sinh(k*ylength*(1+i))).^2+imag(sinh(k*y*(1+i))/sinh(k*ylength*(1+i))).^2).*sin(w*j*(dt)+angle(sinh(k*y*(1+i))/sinh(k*ylength*(1+i))));
...
[vcoef,pcoef]=compute_operator(Edens,levelsV,Re,st,viscmat,viscmatmid,dt);
    ...
    eShear(c3) = sum(sum(sum(Efzt(:,1:Enm,1:Epm))))*h^3/(xlength*zlength)*charLength;  %S.n = t=F/A --- *charLength??
    vShear(c3) =sum(sum(sum(viscmat{1}(Em-11:Emm,1:Enm,1:Epm).*(uz(Em-10:Em,1:Enm,1:Epm)-uz(Em-12:Emmm,1:Enm,1:Epm)))))/(2*h)*h^3/(xlength*10*h*zlength);
    vShear2(c3) =sum(sum(sum(viscmat{1}(Em-5:Emm,1:Enm,1:Epm).*(uz(Em-4:Em,1:Enm,1:Epm)-uz(Em-6:Emmm,1:Enm,1:Epm)))))/(2*h)*h^3/(xlength*5*h*zlength);

	Shear_Force(c3)=-eShear(c3)*h^3-vShear(c3)*h^3;

    [bStrain(c3), fStrain(c3),S] = Calculate_Strains2(A,Xreal,S,ux,uy,uz,h,X0,S0,dt,st,xlength,ylength,zlength,upper,h,x,y,z,numOfnonzero, bthickness);
