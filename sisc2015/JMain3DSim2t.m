...
xlength=5;%diameter of the tube
ylength=5;%radius of the tube
zlength=5;%length of the tube for the computational domain
X(:,3)=X(:,3)-min(X(:,3));
X(:,1)=X(:,1)-min(X(:,1));


h=zlength/64;%64 should be min, this is the hx hy, and hz
...
bthickness=0.0; %thickness of region where bacteria are adhered to walls
% X=X(find(X(:,2)<0.2),:);
Xm=mean(X);

X=X(sqrt((X(:,1)-Xm(1)).^2+(X(:,2)-Xm(2)).^2/4+(X(:,3)-Xm(3)).^2/3)<0.3,:);
X=X-ones(length(X),1)*Xm;
X=X*pca(X);
X=X+ones(length(X),1)*[xlength/2 zlength/2 ylength/2];
...
for c1=1:numOfPoints
    for c2=c1+1:numOfPoints
        ...
        if dtemp<connectdist && dtemp>h/20*0.9
...
load('w1_f50_b0_cnd0.16216_visc500_dt500_Tumbling.mat')
uhalfz=uzm;
for c3=375:numtimesteps   %change to 1 for actual simulation
  ...
  [vcoef,pcoef]=compute_operator(Edens,levelsV,Re,st,viscmat,viscmatmid,dt);
    ...
    ux(1,:,:)=-(1/(1+exp(-5*c3*dt))-1/2);
	%Periodic STrain
    ux(Em,:,:)=(1/(1+exp(-5*c3*dt))-1/2);
...
edenscoef=eust*dt/(2*h)./Edens{1}.Edensin(2:Emm,2:En,2:Ep);
    ...
    U(stuckb,3)=0;
    U(stuckt,3)=ux(Em,2,3); %U(stuckt,1:2)=0;
...
%         save sim3DShroom0_0dens500_0viscFromDataSTW3fmax4dt0001dx1_256_dt_0001_t_01_scl_rc_1.mat
	...
	str4=num2str(1/(dt*w));
	str5=num2str(c3/100);
        runid=['Sim_',str1,'_',str2,'_',str3,'_',str4, '_tumble'];
        save([runid,'.mat']);
    end
