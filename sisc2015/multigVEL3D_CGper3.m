function [vxsave,vysave,vzsave,vzerr]=multigVEL3D_CGper3(dt,h,ux,uy,uz,uxm,uym,uzm,uhalfx,uhalfy,uhalfz,p,Edens,Efx,Efy,Efz,...
...
Eusex=1/2*(Euse(:,2:En,:)+Euse(:,1:Enm,:));
Eusey=1/2*(Euse(2:Em,:,:)+Euse(1:Emm,:,:));
Eusez=1/2*(Euse(:,:,2:Ep)+Euse(:,:,1:Epm));

coefx=eust*dt/h./Eusex;
coefy=eust*dt/h./Eusey;
coefz=eust*dt/h./Eusez;

uhalfz(Em,:,2:Epm)=uz(Em,:,2:Epm)+coefz(Em,:,2:Epm).*(p(Em,:,3:Ep)-p(Em,:,2:Epm));
uhalfz(1,:,2:Epm)=uz(1,:,2:Epm)+coefz(1,:,2:Epm).*(p(1,:,3:Ep)-p(1,:,2:Epm));
% uhalfz(Em,:,Ep)=uz(Em,:,Ep)+coefz(Em,:,Ep).*(p(Em,:,1)-p(Em,:,Epm));
uhalfz(Em,:,1)=uz(Em,:,1)+coefz(Em,:,1).*(p(Em,:,2)-p(Em,:,1));
% uhalfz(1,:,Ep)=uz(1,:,Ep)+coefz(1,:,Ep).*(p(1,:,1)-p(1,:,Epm));
uhalfz(1,:,1)=uz(1,:,1)+coefz(1,:,1).*(p(1,:,2)-p(1,:,1));
uhalfz(:,:,Ep)=uhalfz(:,:,1);

% uhalfy(1,:,:)=uy(1,:,:)+coefy(1,:,:).*(p(2,:,:)-p(1,:,:));
% uhalfy(Em,:,:)=uy(Em,:,:)+coefy(Em,:,:).*(p(Emm,:,:)-p(Emmm,:,:));
uhalfy=uy;

uhalfx(Em,1,:)=ux(Em,1,:)+coefx(Em,1,:).*(p(Em,2,:)-p(Em,1,:));
% uhalfx(Em,En,:)=ux(Em,En,:)+coefx(Em,En,:).*(p(Em,1,:)-p(Em,Enm,:));
uhalfx(Em,2:Enm,:)=ux(Em,2:Enm,:)+coefx(Em,2:Enm,:).*(p(Em,3:En,:)-p(Em,2:Enm,:));
uhalfx(1,2:Enm,:)=ux(1,2:Enm,:)+coefx(1,2:Enm,:).*(p(1,3:En,:)-p(1,2:Enm,:));
uhalfx(1,1,:)=ux(1,1,:)+coefx(1,1,:).*(p(1,2,:)-p(1,1,:));
% uhalfx(1,En,:)=ux(1,En,:)+coefx(1,En,:).*(p(1,1,:)-p(1,Enm,:));
uhalfx(:,En,:)=uhalfx(:,1,:);
