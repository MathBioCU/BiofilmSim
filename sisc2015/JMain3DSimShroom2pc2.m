...
% w=1;
e0=ylength/1.8*4*3.5*10*10^-6*tan(0.13);
...
x1=[Xb, Yb,Zb];
x1=x1(x1(:,1)<30 & x1(:,2)<30 & x1(:,3)<15 & x1(:,2)>0 & x1(:,1)>10 ,:);
x1=[x1(:,1)-10,x1(:,2)-5,x1(:,3)]; %set edge from 10 microns to 0 microns
...
%     ZFb=b*ZrelVel;
    ...
    [Efxs,Efys,Efzs]=transferLtoE3Dper_e2a(h,x,y,z,Xs,sum(Fx,2)+sum(XFb,2),sum(Fy,2)+sum(YFb,2),sum(Fz,2)+sum(ZFb,2),d0mean,zlength,xlength);
    
    
    if addlvisc>0 || addldens>0
        viscmat{1}=transferLtoEvisc3Dper_e2a(h,x,y,z,Xs,visc,addlvisc,numOfnonzero,1,zlength,xlength);
        viscmatmid{1}.lr=(viscmat{1}(:,2:En,:)+viscmat{1}(:,1:En-1,:))/2;
        viscmatmid{1}.ud=(viscmat{1}(2:Em,:,:)+viscmat{1}(1:Em-1,:,:))/2;
        viscmatmid{1}.fb=(viscmat{1}(:,:,2:Ep)+viscmat{1}(:,:,1:Ep-1))/2;
        
     Edens{1}.Edensin=transferLtoEdens3Dper_e2a(h,x,y,z,Xs,initdensity,addldens,d0mean,numOfnonzero,zlength,xlength);
...
[uhalfx,uhalfy,uhalfz,verr(c3)]=multigVEL3D_CGper2(dt/2,h,ux,uy,uz,uxm,uym,uzm,uhalfx,uhalfy,uhalfz,p,Edens,Efxs*fc,Efys*fc,Efzs*fc,...
    ...
    %Now solve for new pressure
    [p,perr(c3)]=multigPRESSUREprod3Dper2(dt/2,h,p,uhalfx,uhalfy,uhalfz,Edens,Em,En,Ep,levelsP,pcoef,eust);
...
%     ZFb=b*ZrelVel;
    ...
    [Efxt,Efyt,Efzt]=transferLtoE3Dper_e2a(h,x,y,z,X(stuckt,:),sum(Fx(stuckt,:),2)+sum(XFb(stuckt,:),2),sum(Fy(stuckt,:),2)+sum(YFb(stuckt,:),2),sum(Fz(stuckt,:),2)+sum(ZFb(stuckt,:),2),d0mean,zlength,xlength);
    ...
    %Transfer the new forces and densities to the Eulerian points
    [Efx,Efy,Efz]=transferLtoE3Dper_e2a(h,x,y,z,X,sum(Fx,2)+sum(XFb,2),sum(Fy,2)+sum(YFb,2),sum(Fz,2)+sum(ZFb,2),d0mean,zlength,xlength);
