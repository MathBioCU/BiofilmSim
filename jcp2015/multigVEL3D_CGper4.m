function [vxsave,vysave,vzsave,vzerr]=multigVEL3D_CGper4(dt,h,ux,uy,uz,uxm,uym,uzm,uhalfx,uhalfy,uhalfz,p,Edens,Efx,Efy,Efz,...
...
resy=zeros(Em,En,Ep);
resz=zeros(Em,En,Ep);
[vxsave,vysave,vzsave]=fmvVEL3Dper4(uhalfx,uhalfy,uhalfz,Re*rhsfixedx,Re*rhsfixedy,Re*rhsfixedz,h,Edens,levels,vcoef);
[resx,resy,resz]=residualvel3Dper3_mex(vxsave,vysave,vzsave,Re*rhsfixedx,Re*rhsfixedy,Re*rhsfixedz,vcoef{1}, resx, resy, resz);

vxerr=max(max(max(abs(resx))));
vyerr=max(max(max(abs(resy))));
vzerr=max(max(max(abs(resz))));

[zx,zy,zz]=fmvVEL3Dper4(zeros(Em,En,Ep),zeros(Em,En,Ep),zeros(Em,En,Ep),resx,resy,resz,h,Edens,levels,vcoef);
...
while (vxerr>allowerr || vyerr>allowerr || vzerr>allowerr) && count1<2 && c<20
    ...
    [Ap0x,Ap0y,Ap0z]=AonVecVEL3Dper2_mex(p0x,p0y,p0z,vcoef{1}); %h,dt,Edens{1},viscmatmid{1},viscmat{1},st,Re);
    ...
    [resx,resy,resz]=residualvel3Dper3_mex(vxnew,vynew,vznew,Re*rhsfixedx,Re*rhsfixedy,Re*rhsfixedz,vcoef{1}, resx,resy,resz);
    vxerr=max(max(max(abs(resx))));
    vyerr=max(max(max(abs(resy))));
    vzerr=max(max(max(abs(resz))));
    ...
    for i=1:0
        [zx,zy,zz]=mvVEL3Dper4(zx,zy,zz,resx,resy,resz,h,Edens,levels,vcoef);
    end
