function Unew=transferEtoLvel3Dper_e3a(h,ux,uy,uz,x,y,z,X,zlength,xlength,r)
Unew=zeros(size(X));
[Em,En,Ep]=size(x);
...
for c1=1:size(X,1)
    ...
    if ximin>xi-10 
        ...
        xdist=abs(h*xlocal-xlength/rc-X(c1,1))/rc;
        ydist=abs(h*ylocal-X(c1,2))/rc;
        zdist=abs(h*zlocal-X(c1,3))/rc;  
     ...
     if zimin>zi-10 
        ...
        xdist=abs(h*xlocal-X(c1,1))/rc;
        ydist=abs(h*ylocal-X(c1,2))/rc;
        zdist=abs(h*zlocal-zlength/rc-X(c1,3))/rc;  
